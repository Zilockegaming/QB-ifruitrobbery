...
local POWER_OUTAGE = "poweroutage"
local POWER_BOX_EXPLOSION = "powerboxexplosion"
...
local requiredItems = {}
local currentSpot = 0
local usingSafe = true

-- Functions

function LockpickDone(success)
    local pos = GetEntityCoords(PlayerPedId())
    local isWearingHandshoes = IsWearingHandshoes()
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and isWearingHandshoes then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            QBCore.Functions.Notify("Your gloves are torn")
        end
...
function GrabItem(spot)
    ...
    QBCore.Functions.Progressbar("grab_ifruititem", "Apparaart ontkoppelen", 10000, false, true, {
...
if usingSafe then
    if not Config.Locations["safe"].isDone then
        ...
        takeAnim()  -- This might be the function causing the animation loop
        -- Add the line below to clear the player's animation loop
...
function ThermiteSuccess()
    QBCore.Functions.Notify("You have successfully hacked the security system!", "success")
...
function ThermiteFailed()
    QBCore.Functions.Notify("Het is je niet gelukt het beveiligingssysteem te hacken!", "error")
    TriggerServerEvent("QBCore:Server:RemoveItem", "thermite", 1)
    TriggerServerEvent("qb-ifruitstore:server:SetSecurityStatus", "isBusy", false)
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Attempted theft at the iFruit Store', coords, true)
end)

RegisterNUICallback('thermitesuccess', function()
    QBCore.Functions.Notify("The fuses are blown", "success")
    TriggerServerEvent("QBCore:Server:RemoveItem", "thermite", 1)
    local pos = GetEntityCoords(PlayerPedId())
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y, Config.Locations["thermite"].z)) < 1.0 then
        TriggerServerEvent("qb-ifruitstore:server:SetSecurityStatus", "isDone", true)
...
RegisterNetEvent("qb-ifruitstore:client:StartThermiteFire", function(coords, maxChildren, isGasFire)
    if #(vector3(coords.x, coords.y, coords.z) - GetEntityCoords(PlayerPedId())) < 100 then
        local pos = vector3(coords.x, coords.y, coords.z - 0.9)
...
RegisterNetEvent('thermite:UseThermite', function()
    if QBCore and LocalPlayer.state["isLoggedIn"] then
        local pos = GetEntityCoords(PlayerPedId())
        local thermiteLocation = vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y, Config.Locations["thermite"].z)
        
        if #(pos - thermiteLocation) < 1.0 then
                    ...
                    if not isCooldown then
                        QBCore.Functions.TriggerCallback("QBCore:HasItem", function(hasItem)
                            if hasItem then
                                ...
                                QBCore.Functions.Notify("You don't have the right equipment!", "error")
                            end
                        end, "thermite")
                    else
                        QBCore.Functions.Notify("This has just been achieved, you will have to wait!", "error")
                    end
                end)
            else
                QBCore.Functions.Notify("There are not enough police officers online (".. Config.RequiredCops ..") to go on!", "error")
            end
        end
    end
end)
...
--             if math.random(1, 100) <= 80 and not IsWearingHandshoes() then
--                 local dropPos = GetEntityCoords(PlayerPedId())
--                 TriggerServerEvent("evidence:server:CreateFingerDrop", dropPos)
--             end
...
--             end
--         else
--             QBCore.Functions.Notify("There are not enough police officers online", "error")
--         end
--     end
-- end)
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y, Config.Locations["thermite"].z)) < 10.0 then
                inRange = true
                local distToThermite = #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y, Config.Locations["thermite"].z))
                if distToThermite < 3.0 and not Config.Locations["thermite"].isDone then
                        ...
                        if not Config.Locations["takeables"][spot].isBusy and not Config.Locations["takeables"][spot].isDone then
                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.RequiredCops then
                                    ...
                                    QBCore.Functions.Notify("At least not enough police online (".. Config.RequiredCops ..") are at least needed!", "error")
                                end
                            end
                        end
                    else
